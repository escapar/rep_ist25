...
BEST_PARAMS = {'ComplexMethod': {'emb_output': 32, 'layers': 1, 'lstm_units': 64, 'epochs': 24, 'dropout': 0.2}, 'ComplexConditional': {'emb_output': 32, 'layers': 1, 'lstm_units': 64, 'epochs': 3, 'dropout': 0.2}, 'FeatureEnvy': {'emb_output': 16, 'layers': 2, 'lstm_units': 64, 'epochs': 16, 'dropout': 0.2}, 'MultifacetedAbstraction': {'emb_output': 16, 'layers': 2, 'lstm_units': 128, 'epochs': 11, 'dropout': 0.2}}

def build_rnn_model(max_features, config):
    tf.keras.backend.clear_session()
    model = Sequential()
    model.add(Embedding(input_dim=max_features + 1, output_dim=config['emb_output'], mask_zero=True))
    for i in range(0, config['layers'] - 1):
        model.add(LSTM(config['lstm_units'], return_sequences=True, recurrent_dropout=0.1, dropout=0.1))
    model.add(LSTM(config['lstm_units'], recurrent_dropout=0.1, dropout=0.1))
...
def evaluate_rnn_rq3(lang, smell, mode, ratio, smoke=False):
    ...
    (clean_train_data, clean_train_labels, clean_eval_data, clean_eval_labels, max_len_clean) = inputs.get_data(clean_base, max_training_samples=train_limit, max_eval_samples=max_eval, is_c2v=False)
    (adv_train_data, adv_train_labels, adv_eval_data, adv_eval_labels, max_len_adv) = inputs.get_data(adv_base, max_training_samples=train_limit, max_eval_samples=max_eval, is_c2v=False)
