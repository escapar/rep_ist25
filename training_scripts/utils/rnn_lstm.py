...
def embedding_lstm(rq1_data, rq2_data, config, smell, rq1_out_folder=RQ1_OUT_FOLDER, rq2_out_folder=RQ2_OUT_FOLDER, dim=DIM):
    ...
    earlystop = tf.keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0.0001, patience=2, verbose=1, mode='auto')
    ...
    model.fit(rq1_data.train_data, rq1_data.train_labels, validation_split=0.2, epochs=config.epochs, batch_size=batch_sizes[b_size], callbacks=callbacks_list)
    ...
    (rq1_auc, rq1_accuracy, rq1_precision, rq1_recall, rq1_f1, rq1_average_precision, rq1_fpr, rq1_tpr) = metrics_util.get_all_metrics(model, rq1_data.eval_data, rq1_data.eval_labels, rq1_y_pred)
    (rq2_auc, rq2_accuracy, rq2_precision, rq2_recall, rq2_f1, rq2_average_precision, rq2_fpr, rq2_tpr) = metrics_util.get_all_metrics(model, rq2_data.eval_data, rq2_data.eval_labels, rq2_y_pred)
    ...
    tf.keras.backend.clear_session()
    return (rq1_auc, rq1_accuracy, rq1_precision, rq1_recall, rq1_f1, rq1_average_precision, rq2_auc, rq2_accuracy, rq2_precision, rq2_recall, rq2_f1, rq2_average_precision, stopped_epoch)
...
def get_all_data(data_path, rq2_eval_data_path, smell):
    print('reading data...')
    (train_data, train_labels, eval_data, eval_labels, max_input_length) = inputs.get_data(data_path, RQ1_OUT_FOLDER, 'rq1_rnn_' + smell, train_validate_ratio=TRAIN_VALIDATE_RATIO, max_training_samples=5000)
    ...
    (training_data, training_labels, eval_data, eval_labels, max_input_length) = inputs.get_data_rq2(data_path, rq2_eval_data_path, RQ2_OUT_FOLDER, 'rq2_rnn_' + smell, train_validate_ratio=TRAIN_VALIDATE_RATIO, max_training_samples=5000)
...
def get_out_file(smell, out_folder, model):
    ...
    return os.path.join(out_folder, model + smell + '_' + str(now.strftime('%d%m%Y_%H%M') + '.csv'))
            ...
            for lstm_units in lstms_units:
                for epoch in epochs:
                    for dropout in dropouts:
                        ...
                        config = configuration.RNN_emb_lstm_config(emb_output=emb_output, lstm_units=lstm_units, layers=layer, epochs=epoch, dropout=dropout)
                        try:
                            start_time = time.time()
                            (rq1_auc, rq1_accuracy, rq1_precision, rq1_recall, rq1_f1, rq1_average_precision, rq2_auc, rq2_accuracy, rq2_precision, rq2_recall, rq2_f1, rq2_average_precision, stopped_epoch) = embedding_lstm(rq1_data, rq2_data, config, smell)
                            end_time = time.time()
                            elapsed_time = end_time - start_time
                            rq1_result_str = str(emb_output) + ',' + str(layer) + ',' + str(lstm_units) + ',' + str(epoch) + ',' + str(stopped_epoch) + ',' + str(rq1_auc) + ',' + str(rq1_accuracy) + ',' + str(rq1_precision) + ',' + str(rq1_recall) + ',' + str(rq1_f1) + ',' + str(rq1_average_precision) + ',' + str(elapsed_time) + '\n'
                            rq2_result_str = str(emb_output) + ',' + str(layer) + ',' + str(lstm_units) + ',' + str(epoch) + ',' + str(stopped_epoch) + ',' + str(rq2_auc) + ',' + str(rq2_accuracy) + ',' + str(rq2_precision) + ',' + str(rq2_recall) + ',' + str(rq2_f1) + ',' + str(rq2_average_precision) + ',' + str(elapsed_time) + '\n'
