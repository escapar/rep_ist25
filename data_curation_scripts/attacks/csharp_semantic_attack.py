...
CSHARP_KEYWORDS = {
    ...
    'nameof', 'notnull', 'on', 'orderby', 'partial', 'remove', 'select',
...
def process_file_pair(args):
    ...
    try:
        ...
        file_mapping = build_file_mapping(all_code_text, CSHARP_KEYWORDS)
        modified = []
        for line in lines:
            modified.append(process_line(line, CSHARP_KEYWORDS, file_mapping))
...
def run_attack(mode):
    print(f'--- C# Synonym Attack: Mode [{mode.upper()}] ---')
    ...
    for smell in smells:
        for case in ['Positive', 'Negative']:
            src = os.path.join('data/csharp_subset_unique', smell, case)
