...
def _build_project(dir, dir_path):
    ...
    if os.path.exists(pom_path):
        print('Found pom.xml')
        os.chdir(dir_path)
        proc = Popen(['C:\\Program Files (x86)\\apache-maven-3.6.2\\bin\\mvn.cmd', 'clean', 'install', '-DskipTests'])
        proc.wait()
        is_compiled = True
    ...
    if os.path.exists(gradle_path):
        ...
        os.chdir(dir_path)
        proc = Popen(['C:\\Program Files\\Gradle\\gradle-5.6.2\\bin\\gradle.bat', 'compileJava'])
        proc.wait()
        is_compiled = True
    if not is_compiled:
