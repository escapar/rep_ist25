...
def _get_positive_src_file_list(lines, solution, smell_type, code_split_out_folder_class, code_split_out_folder_method):
    pos_src_file_list = list()
    for line in lines:
        tokens = line.split(',')
        if smell_type == 'Design':
            file = os.path.join(os.path.join(os.path.join(os.path.join(os.path.join(code_split_out_folder_class, solution), solution), str(_filter_str(tokens[1]))), str(_filter_str(tokens[2])).replace('<', '').replace('>', '') + '.code'))
        else:
            file = os.path.join(os.path.join(os.path.join(os.path.join(os.path.join(code_split_out_folder_method, solution), solution), str(_filter_str(tokens[1]))), str(_filter_str(tokens[2])).replace('<', '').replace('>', '')), str(_filter_str(tokens[3])) + '.code')
        ...
        for file in files:
            if smell_type == 'Design':
                if not file.endswith('DesignSmells.csv'):
                    continue
            elif not file.endswith('ImplementationSmells.csv'):
                continue
            ...
            with open(os.path.join(root, file), encoding='utf8', errors='ignore') as fp:
                for line in fp:
                    if smell_name_str in line:
                        lines.append(line)
            pos_source_files = _get_positive_src_file_list(lines, solution, smell_type, code_split_out_folder_class, code_split_out_folder_method)
