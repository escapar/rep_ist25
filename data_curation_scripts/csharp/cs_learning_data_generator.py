...
def _get_positive_src_file_list(lines, solution, project, folder_path, smell_type, code_split_out_folder_class, code_split_out_folder_method):
    ...
    for line in lines:
        ...
        if smell_type == 'Design':
            file = os.path.join(os.path.join(os.path.join(os.path.join(code_split_out_folder_class, solution), str(_filter_str(project))), str(_filter_str(tokens[1]))), str(_filter_str(tokens[2])).replace('<', '').replace('>', '') + '.code')
        else:
            file = os.path.join(os.path.join(os.path.join(os.path.join(os.path.join(code_split_out_folder_method, solution), project), tokens[1]), tokens[2].replace('<', '').replace('>', '')), tokens[4] + '.code')
...
def _put_files_in_right_bucket(pos_source_file_list, solution, positive_cases_folder, negative_cases_folder, smell_type, code_split_out_folder_class, code_split_out_folder_method):
    ...
    base_folder_path = os.path.join(code_split_out_folder_class, solution) if smell_type == 'Design' else os.path.join(code_split_out_folder_method, solution)
    pos_counter = 0
    neg_couter = 0
        ...
        for file in files:
            ...
            if _is_present(pos_source_file_list, src_file_path):
                ...
                if not os.path.exists(dest_file_path):
                    ...
                    print('File already exists: ' + str(dest_file_path))
            else:
...
def _scan_solution(solution, positive_cases_folder, negative_cases_folder, smell_name_str, smell_type, smells_results_folder, code_split_out_folder_class, code_split_out_folder_method):
        ...
        for file in files:
            if smell_type == 'Design':
                if not file.endswith('DesignSmells.csv'):
                    continue
            elif not file.endswith('ImpSmells.csv'):
                continue
            ...
            pos_source_files = _get_positive_src_file_list(lines, solution, project, positive_cases_folder, smell_type, code_split_out_folder_class, code_split_out_folder_method)
            pos_source_file_list.extend(pos_source_files)
    total_copied_files = _put_files_in_right_bucket(pos_source_file_list, solution, positive_cases_folder, negative_cases_folder, smell_type, code_split_out_folder_class, code_split_out_folder_method)
...
def generate_data(smells_results_folder, code_split_out_folder_class, code_split_out_folder_method, learning_data_folder_base):
    ...
    for smell in range(len(SMELL_NAME_LIST)):
        ...
        for solution in os.listdir(smells_results_folder):
            ...
            _scan_solution(solution, positive_cases_folder, negative_cases_folder, SMELL_NAME_STR_LIST[smell], SMELL_TYPE_LIST[smell], smells_results_folder, code_split_out_folder_class, code_split_out_folder_method)